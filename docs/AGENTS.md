# AGENTS.md — Règles pour l'agent Cursor sur le projet Zola
# Ce fichier est chargé automatiquement par Cursor à chaque interaction (racine du projet).
# docs/CONTEXT.md et docs/SPECS.md ne le sont PAS — réfère-les explicitement avec
# @docs/CONTEXT.md et @docs/SPECS.md au début de chaque prompt de ticket (voir PROMPTS.md).

## Règle d'or

**Avant tout ticket, lire entièrement `docs/CONTEXT.md` et `docs/SPECS.md`** (via `@docs/CONTEXT.md` et `@docs/SPECS.md` dans le prompt — ce fichier-ci est automatique, ces deux-là ne le sont pas). Ne jamais coder à partir d'une supposition — tout ce dont tu as besoin est dans ces deux fichiers ou dans le ticket lui-même.
...
## Spécificités Cursor (agent, pas juste autocomplétion)

- **Checkpoint git avant tout ticket touchant plusieurs fichiers** : `git add -A && git commit -m "checkpoint: avant TICKET-XX"`. Permet de revenir en arrière proprement si l'agent part dans une mauvaise direction sur un ticket complexe (TXN-04, PT-02, CL-01 notamment).
- **Jamais de commit automatique sans relecture** : l'agent code et teste, mais le commit final de validation d'un ticket reste une action humaine.
- **Sur les fichiers longs** (`PROMPTS.md` lui-même, ou un futur `Controller` qui grossit) : préférer des remplacements ciblés à une réécriture complète du fichier, pour limiter les erreurs de diff.
- Si l'agent détecte une incohérence entre deux documents (ça arrivera malgré les 4 audits déjà faits) : s'arrêter et signaler plutôt que de trancher seul silencieusement.
